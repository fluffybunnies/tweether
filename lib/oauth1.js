...
module.exports = function(url, params, method){
	...
	;
	return {
		urlGet: baseUrl + (paramsStr ? (url.indexOf('?') == -1 ? '?' : '&')+paramsStr : '')
...
function getAuthParams() { 
	return {
		...
		,oauth_nonce: config.twitterNonce || crypto.createHash('md5').update(Math.random()+'').digest('hex')
		,oauth_signature_method: 'HMAC-SHA1'
		,oauth_timestamp: config.twitterTimestamp || Math.round(Date.now()/1000)
...
function generateSignature(baseUrl, method, paramsStr){
	...
	,key = ut.encode_rfc3986(config.twitterConsumerSecret)+'&'+ut.encode_rfc3986(config.twitterAccessTokenSecret)
	;
